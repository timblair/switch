...
describe Switch::Deck do
  ...
  describe "#sort!" do
    ...
    it "sorts the cards correctly" do
      expect(deck.sort!).to eq cards.sort
    end
  end

  describe "#sort" do
    let(:cards) { 5.times.map { Switch::Card.random } }
    let(:sorted) { deck.sort }
    before(:each) { deck << cards }

    it "sorts the cards correctly" do
      expect(sorted).to eq deck.sort
    end

    it "returns a new Deck instance" do
      expect(sorted.object_id).not_to eq deck.object_id
    end
  end

  describe "#shuffle!" do
    let(:cards) { 5.times.map { Switch::Card.random }.sort }
    before(:each) { deck << cards }

    it "shuffles the card order" do
      expect(deck.shuffle!).to_not eq cards
    end
  end
  ...
  describe "#shuffle" do
    ...
    it "returns a new Deck instance" do
      expect(shuffled.object_id).not_to eq deck.object_id
    end
  end
